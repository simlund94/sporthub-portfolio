...
import {
    useLeagueByIdWithEvents, useLeagueStandingsById, useTeamsByLeagueId, useTeamFormsByLeagueId
...
const LeagueStandingsTable = ({leagueId}) => {
    ...
    const {
        data: eventsData, loading: eventsLoading, err: eventsError,
    } = useLeagueByIdWithEvents(leagueId, "UPCOMING", fromDate, toDate);

    const {
        data: teams, loading: teamsLoading, err: teamsErr,
    ...
    return (<div>
            <table className="hidden md:table mx-auto table-zebra w-full table-pin-cols table-pin-rows">
                ...
                {data.map((teamItem) => {
                    ...
                    const teamEvents = eventsData?.filter((ev) => ev.homeTeam?.id === teamId || ev.visitingTeam?.id === teamId) || [];
                    ...
                    const nextMatchLogo = opponent?.logo ? (<div className="flex items-center gap-2">
                            <img
                                src={opponent.logo}
            ...
            <div className="overflow-x-auto md:hidden">
                <table className="table mx-auto table-zebra w-full">
                    ...
                    {data.map(teamItem => {
                        ...
                        return (<tr key={teamId} className="cursor-pointer hover:bg-base-200"
                                    onClick={() => navigate(`/team/${teamId}`)}>
                                <td className="flex items-center gap-4">
