...
const LeaguePage = ({initialLeagueId}) => {
    ...
    if (seasonsLoading || standingsLoading) return <div>Loading...</div>;
    if (seasonsErr || standingsErr) return <div>Error loading data</div>;

    const {leagueName, allSeasons} = seasonData;
    const currentSeason = allSeasons?.find(s => s.id === leagueId)?.slug || allSeasons?.[0]?.slug;
